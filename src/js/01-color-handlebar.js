...
// шаблон одного єлемента
// импортим + шаблонизируем под функцию мап
import colorCardsTpl from "../templates/color-cards.hbs";
// шаблон массива єлементов
// импортим + шаблонизируем без функции мап
import "../css/01-color-handlebar..css";
import colors from "../js/01-colors.json";
// при импорте оно сразу его парсит в JS массив из JSON
// const colors = [
...
// ];
console.log(`colorCardsTpl`, colorCardsTpl(colors));
console.log(`colorCardTpl`, colorCardTpl(colors[1]));
// в парселе импорт сразу становиться функцией шаблоном
...
const refs = {
  ...
  createColorCardEl(colors) {
    ...
    // мепаю колор и на каждой итерации запускаю шаблонизатор и на результат делаю join('')  ==>  получаем струку
    // return colors.map(colorCardTpl).join("");
    // более правильный вариант записи
    // ---------------------------------------------
    // ---------------------------------------------
    return colorCardsTpl(colors);
    //шаблонизатор который принимает массив и его до конца делает (не нужен мап + джойн)
    // ---------------------------------------------
    // ---------------------------------------------
    //   return colors
    //     .map(({ hex, rgb }) => {
